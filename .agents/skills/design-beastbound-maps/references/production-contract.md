...
- 可选 `ground.pathTransitionTileIds` 只用于道路边缘与草地的视觉衔接，必须为四条等距边 `nw/ne/sw/se` 的全部 15 种非空暴露组合各声明一张独立完整地砖：`nw`、`ne`、`nw_ne`、`sw`、`nw_sw`、`ne_sw`、`nw_ne_sw`、`se`、`nw_se`、`ne_se`、`nw_ne_se`、`sw_se`、`nw_sw_se`、`ne_sw_se`、`nw_ne_sw_se`；四边对应格偏移 `[-1,0]`、`[0,-1]`、`[0,1]`、`[1,0]`。运行时只在语义仍为 `pathTileId`、且相邻格不是道路／广场／warp 的暴露边上，于 prepare 阶段按精确组合选择，不得在多边拐角丢弃其中一边。它只能改最终视觉 `tileIdsByCell`、`tileCounts` 与诊断计数，绝不得改 semantic lookup、道路／广场／warp 集、碰撞、寻路、遭遇、protected 或权威地图 JSON。缺少语义 base、任一组合、独立 tile，或出现重复、未知、语义／变体复用必须失败关闭；draw 热路径不得随机选择。只有四张单边图的 2×2 母表仍是中间证据，不得作为正式过渡集发布。
- 可选 `ground.plazaTransitionTileIds` 以同一 15 组合合同只处理 `plazaTileId` 广场／石坪的草地过渡边。所有 tile 必须互不重复、已注册、不得复用语义／变体 tile，并且不得与 `pathTransitionTileIds` 复用。该后处理只改最终视觉 tile 与独立的 `plazaTransitionCount`，不得改变任何道路、广场、warp、碰撞、寻路、遭遇、protected 或权威地图事实。
...
- 提升仅选中 bundle 的地图，review 目录中其他候选保持待审，不随整份目录进入 primary。冻结报告的 `catalogSha256` 保留采集时目录哈希；当前成员、manifest／binding 路径、binding 与权威 map-data 哈希另行逐项验证，不因无关目录增删而改写历史证明。
