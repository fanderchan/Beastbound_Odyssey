...
- collision 与 performance 正式报告必须带精确三键 `runnerIdentity: {runner: "godot", runnerVersion, buildIdentity}`，后两项必须非空；并以 `rawRunnerReceipt: {path, sha256}` 冻结非空 `.log`、`.txt` 或 `.jsonl` 原始 runner 输出。pending 阶段可暂缺，但缺少任一项就是正式 release blocker；摘要 JSON、终端转述或人工改写数值不能代替原始 receipt。
...
- lifecycle 候选变更必须先在未分发的工作树/构建候选中完成，再运行上述门禁；任一检查失败就恢复 `approved` 或 pending 且保持两项运行时开关 false。禁止让运行时字段自行宣称证据有效，禁止在 pre-export 门禁之外手工把包翻成 `released`。
