...
- Client growth and server EXP/stat settlement are not yet one complete P0.2 truth. Battle pets, riding pets, and world pet-EXP items now route through one server dispatcher, but authority-v1 is deliberately disabled until public response projection, client no-reroll handling, cache cleaning, and protocol v2 switch atomically. Legacy pets keep level/EXP-only behavior; v1, unknown, or damaged authority state fails closed without falling back to the legacy writer.
- P0.2 now has cross-runtime shadow growth, a strict server growth catalog, a pure deterministic settlement runtime, server-derived model markers, idempotent public pet/profile projections, offline-safe server-cache cleaning, CSPRNG primitives, and a default-off production EXP dispatcher. The four current production pet creation paths use CSPRNG identity and persist true Lv1 facts when known, but response projection, cache cleaning, no-reroll routing, and v1 creation/settlement activation still require their staged atomic cutover.
