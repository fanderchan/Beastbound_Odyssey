...
| Species-relative public Lv1 percentile calculation | Server authority/filter: `server/node/src/auth/pet-level-one-percentile.js`; owned-pet presentation parity: `client/godot/scripts/progression/pet_level_one_percentile_model.gd`; offline all-profile check: `tools/pet_level_one_percentile_audit.mjs` |
...
- The Godot v2 login path cleans both server caches but never loads either before the first fresh pull. Every full server profile passes strict projection, marker-aware no-RNG normalization, and dedicated public-cache publication. Growth UI shows a permanent Lv1 4V species-percentile chart beside a separate observed-growth chart; it uses only Lv1/current evidence and observed grades, never an exact hidden Lv140 result or a combined quality score.
