...
Phase57 是 Godot 4.7 本地遇敌循环、挂机走动和遇敌石原型阶段：
...
- 人物指令后，同一个右上面板会切换成受控宠物的 `PET` 面板。
- PET 面板已有 `技1 攻击`、`技2 防御`、`技3 布伊冲撞`；敌方目标技能走同一套悬停/点击/触摸选目标流程。
...
- PET 技能槽现在从行动目录读取标签和效果元数据。
- PET `技4 催眠粉`、`技5 迷惑吼`、`技6 石化凝视` 可以点选目标，并结算为 `skill_status` 事件。
- `--battle-status-skill-test` 会打开用于测试新 PET 状态技能的 10v10 手工测试战斗。
...
- 被动提示条层级低于右上 `PLAYER` / `PET` 操作框；二者重叠时，操作框在上层。
...
- 战斗行动播放期间，右上 `PLAYER`/`PET` 指令面板会自动隐藏；回到可下指令阶段再出现。
- `PLAYER` 面板保持固定 2x4 布局；`PET`、精灵、物品、换宠面板使用固定宽度的纵向菜单。
...
在这个场景里，人物先任意选择一个行动，然后用 PET `技4 催眠粉`、`技5 迷惑吼` 或 `技6 石化凝视` 点选敌方目标。
...
在这个场景里，人物先任意选择一个行动，然后用 PET `技4 催眠粉`、`技5 迷惑吼` 或 `技6 石化凝视` 点选不同敌方目标。例如 `技4 催眠粉` 点 `普通乌力` 应稳定上 `眠`，点 `厚皮乌力` 应显示 `抵抗`。
...
在这个场景里，`普通猎人` 开局带 `毒` 和 `眠`，可以用 `物品` -> `净化草5` 点他来解除；`高速乌力` 开局带 `眠`，可用 PET `技6 石化凝视` 覆盖成 `石`；`高防乌力` 来自乌力系 10地 形态，因 `硬壳体质` 表现为石化免疫，点它应显示 `免疫`。鼠标移到高防乌力身上，顶部应显示 `硬壳体质：[被动技能] 根据地属性获得石化抗性；地属性达到10时免疫石化。`
