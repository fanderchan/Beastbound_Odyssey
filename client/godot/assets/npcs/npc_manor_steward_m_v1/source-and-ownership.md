...
- 本外观包于 2026-07-21 至 2026-07-22 为 Beastbound Odyssey 定向创建，创意像素来自 OpenAI 内置 `image_gen`；逐字提示词在生成前冻结于唯一正式包的 `source/prompts/`。
- 项目已有兽栏管理员与银行管理员身份板只用作线条、软赛璐璐明暗、成人比例和整体完成度参考；未复制其人物、脸、服装、道具或姿势。
- 无损原稿、原始生成时间、内置生成归档路径、文件与 decoded RGBA 哈希、背景实测和所有权依据均封装在唯一正式包的 `source/` 与 ledger 文件中。
- 缺失逐字 prompt 的早期图像和触碰单元格边界的 v2 世界图均已拒绝，只保存在本地前期生产档案，不参与正式处理或本次冻结交付。
- `image_gen` 实际生成的是空间变化的近洋红全不透明背景，而非精确纯 `#FF00FF`；不得使用事后全图色距或猜测性去底。
- 2026-07-21 透明精确编辑实验在收到停机指令后终止，无候选输出落盘；对应 prompt 仅作为实验记录，不能冒充原稿或透明来源。
