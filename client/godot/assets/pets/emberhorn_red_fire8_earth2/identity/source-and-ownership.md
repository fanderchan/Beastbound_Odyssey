...
- 资产范围：身份板与四个关键姿势；真八向世界 `idle 1 + walk 4` 共 40 个运行帧；两个正式战斗视角的 12 动作、每动作 4 帧，共 96 个运行帧及对应 512px 源帧。
- 制作日期：2026-07-19。
...
- 完整动作证据：`../prompts/full-production-generation-contract-v1.md`；世界与战斗原始表分别归档于 `../source/world-raw/` 和 `../source/battle-raw/`，处理参数与 512px 源帧位于相邻 `world-pipeline`、`battle-pipeline`、`world-frames`、`battle-frames` 目录。
...
- 发布状态：`self_review_passed_owner_pending`。独立宠物世界/战斗矩阵已完成并保持 `runtimeEnabled=false`；骑乘矩阵、真实 Godot 画面与项目所有者审美验收仍是后续门禁。
