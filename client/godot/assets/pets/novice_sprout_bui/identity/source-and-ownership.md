...
- 可替换性：先按 `identity-lock.md` 重建身份板，再按动作节拍生成色键母版，使用项目切帧/归一化工具输出 512px QA 母版与 256px 运行帧；随后必须重新通过 68 帧资产门禁、Godot 实机截图、MP4 连贯性和性能验收。
- 发布状态：工程与实机自评通过，项目所有者视觉验收待定；在确认风格、比例和动作语言前，不据此批量生产其他宠物。
