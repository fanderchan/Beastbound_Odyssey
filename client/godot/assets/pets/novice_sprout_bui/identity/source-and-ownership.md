...
- 资产范围：身份板、正/背三分之四战斗视角，以及 `idle/walk/attack/hurt/defend/stagger/down` 两视角动作帧；另含世界 `independent_8` 方向的 `idle/walk` 动作帧。
- 制作日期：2026-07-18。
...
- 战斗兼容合同：既有正/背三分之四两视图与 100 张宠物战斗帧保持不变，世界真八方向不会改变己方/敌方的战斗朝向映射。
...
- 发布状态：工程与 Phase 308 实机自评通过；Phase 306 的昏厥基础和 Phase 308 的前爪攻击/负伤退行证据分别位于 `.run/evidence/phase306_stoneage_battle_foundation/`、`.run/evidence/phase308_counter_wounded_return/`。世界真八方向也只完成工程自评；项目所有者视觉验收仍为 `pending`。在确认风格、比例、方向语义、昏厥语义和动作语言前，不据此批量生产其他宠物。
