...
func _run_auto_auth_server_client_check() -> void:
	...
	var parsed_equipment_synthesis = ServerAuthClientModel.parse_equipment_synthesize_response(200, JSON.stringify({
		...
		"synthesis": {
			"recipeId": "craft_hardwood_club",
			"outputItemId": "weapon_hardwood_club",
			"outputCount": 1,
			"stoneCost": 20,
		},
		"questMessages": ["任务更新：合成装备 1/1。"],
	}).to_utf8_buffer())
			...
			"actors": [
				{
					...
					"hp": 120,
					"maxHp": 120,
					"ridePetInstanceId": "ride_test",
					"ridePetName": "骑宠",
					"ridePetFormId": "bui_normal_yellow_wind10",
					"ridePetHp": 80,
					"ridePetMaxHp": 90,
					"ridePetBattleState": "riding",
				},
				{
					"actorId": "srv_enemy_wild",
					"side": "enemy",
					"kind": "wild_pet",
					"displayName": "野生乌力",
					"slotId": "enemy.back.2",
...
func _run_auto_server_click_move_live_check() -> void:
	...
	if path_cells.size() < 2:
		goal_cell = IsoMapModel.nearest_walkable_cell(host.map_data, start_cell + Vector2i(0, 3))
		path_cells = IsoMapModel.find_path(host.map_data, start_cell, goal_cell)
	...
	var seed_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
	...
	var target_screen = host._world_to_screen(IsoMapModel.grid_to_world(host.map_data, goal_cell))
	host._set_click_move_target(target_screen)
	var guard = 0
	while guard < 900 and (
		host.server_step_move_active
		or host.server_step_move_request_pending
		or host.server_step_move_waiting_for_visual
		or (host.player != null and host.player.is_auto_moving())
	):
		guard += 1
		await host.get_tree().process_frame
	...
	var publish_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(session.get("serverSessionToken", "")),
		{
			"mapId": host.current_map_id,
			"cellX": final_cell.x,
			"cellY": final_cell.y,
			"facing": host.player.get_facing_key() if host.player != null and host.player.has_method("get_facing_key") else "south",
			"moving": false,
		}
	))
	...
	print("server click move live check ready: status=%s register=%s seed=%s local_click=%s publish=%s expected_steps=%d requests=%d acks=%d final=%s goal=%s error=%s user=%s register_http=%d register_result=%s register_body=%d" % [
		...
		str(final_cell),
		str(goal_cell),
		host.server_step_move_last_error_code,
		username,
...
func _run_auto_server_click_move_reject_live_check() -> void:
	...
	var username = host._live_check_username("cmr")
	var register_response = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		username,
		"test1234",
	...
	host.encounter_zone_step_count = 0
	host.encounter_grace_remaining = 999.0
	var stale_cell = IsoMapModel.nearest_walkable_cell(host.map_data, IsoMapModel.spawn_cell(host.map_data) + Vector2i(7, -1))
	var server_cell = IsoMapModel.nearest_walkable_cell(host.map_data, stale_cell + Vector2i(1, 0))
	if server_cell == stale_cell:
		server_cell = IsoMapModel.nearest_walkable_cell(host.map_data, stale_cell + Vector2i(0, 1))
	var goal_cell = IsoMapModel.nearest_walkable_cell(host.map_data, server_cell + Vector2i(3, 0))
	var server_path_cells: Array[Vector2i] = IsoMapModel.find_path(host.map_data, server_cell, goal_cell)
	if server_path_cells.size() < 2:
		goal_cell = IsoMapModel.nearest_walkable_cell(host.map_data, server_cell + Vector2i(0, 3))
		server_path_cells = IsoMapModel.find_path(host.map_data, server_cell, goal_cell)
	var expected_acks = maxi(0, server_path_cells.size() - 1)
	if host.player != null:
		host.player.global_position = IsoMapModel.grid_to_world(host.map_data, stale_cell)
		host.player.clear_move_target()
	host.server_step_move_authority_valid = false
	host._cancel_server_step_move()
	var seed_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
	...
	var seed_parsed = ServerAuthClientModel.parse_player_position_update_response(int(seed_response.get("responseCode", 0)), seed_response.get("body", PackedByteArray()) as PackedByteArray)
	var seed_ok = bool(seed_parsed.get("ok", false))
	host.server_step_move_authority_cell = stale_cell
	host.server_step_move_authority_valid = true
	host.server_step_move_sync_retry_count = 0
	var target_screen = host._world_to_screen(IsoMapModel.grid_to_world(host.map_data, goal_cell))
	host._set_click_move_target(target_screen)
	var guard = 0
	while guard < 900 and (
	...
	var reject_recovered = (
		...
		and host.server_step_move_request_count >= expected_acks + 1
		and host.server_step_move_ack_count >= expected_acks
		and host.server_step_move_last_error_code == ""
		and final_cell == goal_cell
	...
	print("server click move reject live check ready: status=%s register=%s seed=%s recovered=%s expected_acks=%d requests=%d acks=%d retries=%d stale=%s server=%s final=%s authority=%s goal=%s error=%s user=%s register_http=%d register_result=%s register_body=%d" % [
		...
		host.server_step_move_request_count,
		host.server_step_move_ack_count,
		host.server_step_move_sync_retry_count,
		str(stale_cell),
		str(server_cell),
		str(final_cell),
		str(authority_cell),
...
func _run_auto_online_aoi_live_check() -> void:
	...
	var watcher_register = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		watcher_username,
		"test1234",
		"视野甲"
	))
	var near_register = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		near_username,
		"test1234",
		"视野乙"
	))
	var far_register = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		far_username,
		"test1234",
		"远处丙"
	...
	if not register_ok:
		...
		host.get_tree().quit(1)
		return
	var watcher_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(1, -1)
	...
	var near_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
	...
	var far_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
	...
	host.current_account_session = watcher_session
	host.current_account_session["serverBaseUrl"] = ServerAuthClientModel.DEFAULT_BASE_URL
	host.account_authenticated = true
	host.server_profile_sync_state = "ready"
	host.player.global_position = IsoMapModel.grid_to_world(host.map_data, watcher_cell)
	host.player.face_direction(Vector2.RIGHT)
	host.last_checked_player_cell = watcher_cell
	host.online_position_remote_players.clear()
	host.online_position_draw_signature_cache = ""
	host._request_online_position_snapshot()
	var frames = 0
	while frames < 720 and host.online_position_request_pending:
		frames += 1
		await host.get_tree().process_frame
	var near_visible = host._online_remote_player_at(near_username, host.current_map_id, near_cell)
	var far_hidden = not host._online_remote_player_at(far_username, host.current_map_id, far_cell)
	var snapshot_ready = true
	var far_still_outside = far_cell + Vector2i(1, 0)
	var far_outside_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(far_session.get("serverSessionToken", "")),
		{
			"mapId": host.current_map_id,
	...
	while frames < 720 and host.online_position_request_pending:
		frames += 1
		await host.get_tree().process_frame
	var outside_event_hidden = bool(far_outside_parsed.get("ok", false)) and not host._online_remote_player_at(far_username, host.current_map_id, far_still_outside)
	var far_near_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(far_session.get("serverSessionToken", "")),
		{
			"mapId": host.current_map_id,
	...
	while frames < 720 and host.online_position_request_pending:
		frames += 1
		await host.get_tree().process_frame
	var moved_in_visible = bool(far_near_parsed.get("ok", false)) and host._online_remote_player_at(far_username, host.current_map_id, moved_near_cell)
...
func _run_auto_server_event_live_check() -> void:
	...
	var actor_register = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		actor_username,
		"test1234",
		"事件乙"
	...
	var watcher_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(1, -1)
	var actor_cell = watcher_cell + Vector2i(2, 0)
	var watcher_position = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(watcher_session.get("serverSessionToken", "")),
		{
			"mapId": host.current_map_id,
			"cellX": watcher_cell.x,
			"cellY": watcher_cell.y,
			"facing": "east",
			"moving": false,
		}
	))
	var watcher_position_parsed = ServerAuthClientModel.parse_player_position_update_response(
		int(watcher_position.get("responseCode", 0)),
	...
	var latest_response = await host._auto_http_request_spec(ServerAuthClientModel.event_latest_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(watcher_session.get("serverSessionToken", ""))
	))
	...
	host.player.face_direction(Vector2.RIGHT)
	host.last_checked_player_cell = watcher_cell
	host.server_event_last_seq = int(latest_parsed.get("latestEventSeq", 0))
	...
	while frames < 720 and not host._server_event_type_seen("events.ready"):
		frames += 1
		await host.get_tree().process_frame
	var ready_ok = host._server_event_type_seen("events.ready")
	frames = 0
	while frames < 720 and not host._server_event_type_seen("online.snapshot"):
		frames += 1
		await host.get_tree().process_frame
	...
	while frames < 120:
		...
		await host.get_tree().process_frame
	host.server_event_seen.clear()
	var actor_position = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
	...
	frames = 0
	while frames < 720 and not host._online_remote_player_at(actor_username, host.current_map_id, actor_cell):
		frames += 1
		await host.get_tree().process_frame
	var position_seen = host._server_event_type_seen("online.position")
	var position_event_username = ""
	var position_event_player_count = 0
	var position_event_has_actor = false
	for seen_event in host.server_event_seen:
		if seen_event is Dictionary and str((seen_event as Dictionary).get("type", "")) == "online.position":
			position_event_username = str((seen_event as Dictionary).get("username", ""))
			var event_players: Array = (seen_event as Dictionary).get("players", []) if (seen_event as Dictionary).get("players", []) is Array else []
			position_event_player_count = event_players.size()
			for event_player in event_players:
				if event_player is Dictionary and str((event_player as Dictionary).get("username", "")) == actor_username:
					position_event_has_actor = true
					break
	var position_event_ok = (
		bool(actor_position_parsed.get("ok", false))
		and position_seen
		and position_event_username == actor_username
		and position_event_has_actor
	)
	var chat_text = "事件消息%s" % suffix
	var actor_chat = await host._auto_http_request_spec(ServerAuthClientModel.chat_send_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(actor_session.get("serverSessionToken", "")),
		CHAT_CHANNEL_NEARBY,
		chat_text
	...
	frames = 0
	while frames < 720 and not host._chat_message_text_seen(chat_text):
		frames += 1
		await host.get_tree().process_frame
	var chat_event_ok = bool(actor_chat_parsed.get("ok", false)) and host._server_event_type_seen("chat.message") and host._chat_message_text_seen(chat_text)
	...
	print("server event live check ready: status=%s register=%s watcher_position=%s latest=%s ready=%s snapshot=%s position=%s actor_http=%s position_seen=%s position_user=%s event_players=%d event_has_actor=%s chat=%s state=%s watcher=%s actor=%s" % [
		...
		str(bool(actor_position_parsed.get("ok", false))),
		str(position_seen),
		position_event_username,
		position_event_player_count,
		str(position_event_has_actor),
		str(chat_event_ok),
		host.server_event_state,
		watcher_username,
		actor_username,
	])
	host._stop_server_event_stream()
...
func _run_auto_battle_room_live_check() -> void:
	...
	var opponent_session = opponent_parsed.get("session", {}) as Dictionary if opponent_parsed.get("session", {}) is Dictionary else {}
	var register_ok = bool(challenger_parsed.get("ok", false)) and bool(opponent_parsed.get("ok", false))
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(4, -1)
	...
	var state_response = await host._auto_http_request_spec(ServerAuthClientModel.battle_state_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(opponent_session.get("serverSessionToken", ""))
	))
...
func _run_auto_server_battle_turn_live_check() -> void:
	...
	while frames < 720 and not host._battle_room_ready(room_id):
		frames += 1
		await host.get_tree().process_frame
	var room_ready_ok = bool(accept_parsed.get("ok", false)) and room_id != "" and host._battle_room_ready(room_id)
	var actor_ids = host._auto_server_battle_duel_actor_ids(accepted_room, challenger_username, opponent_username)
	var challenger_player_actor_id = str(actor_ids.get("challengerPlayer", ""))
	...
	var playback_ok = (
		host.battle_active
		and BattleModel.uses_10v10_formation(host.battle_state)
		and int(host.battle_state.get("round", 0)) == 2
		and host.battle_last_round_applied_events >= turn_events.size()
		and host.battle_last_round_event_types.has("attack")
		and host.battle_last_round_event_types.has("defend")
		and host.battle_last_event_damage >= 0
	)
	var room = host.server_battle_state.get("room", {}) as Dictionary if host.server_battle_state.get("room", {}) is Dictionary else {}
...
func _run_auto_server_battle_reconnect_live_check() -> void:
	...
	var opponent_session = opponent_parsed.get("session", {}) as Dictionary if opponent_parsed.get("session", {}) is Dictionary else {}
	var register_ok = bool(challenger_parsed.get("ok", false)) and bool(opponent_parsed.get("ok", false))
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(6, -1)
	var challenger_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(challenger_session.get("serverSessionToken", "")),
		{
	...
	var accepted_room = accept_parsed.get("room", {}) as Dictionary if accept_parsed.get("room", {}) is Dictionary else {}
	var room_id = str(accepted_room.get("roomId", ""))
	var room_ready_ok = bool(invite_parsed.get("ok", false)) and bool(accept_parsed.get("ok", false)) and room_id != ""
	host.current_account_session = opponent_session
	host.current_account_session["serverBaseUrl"] = ServerAuthClientModel.DEFAULT_BASE_URL
	host.account_authenticated = true
	host.server_profile_sync_state = "ready"
	...
	var state_parsed = ServerAuthClientModel.parse_battle_state_response(int(state_response.get("responseCode", 0)), state_response.get("body", PackedByteArray()) as PackedByteArray)
	var restored_room = state_parsed.get("room", {}) as Dictionary if state_parsed.get("room", {}) is Dictionary else {}
	var state_ok = bool(state_parsed.get("ok", false)) and str(restored_room.get("roomId", "")) == room_id
	var restored_ok = host._apply_server_battle_room_state(restored_room, true)
	await host.get_tree().process_frame
	var ally_actor = BattleModel.actor_by_id(host.battle_state, BattleModel.PLAYER_ACTOR_ID)
	var enemy_actor = BattleModel.actor_by_id(host.battle_state, "enemy_player")
	var visible_ok = (
		restored_ok
		and host.battle_active
		...
		and BattleModel.uses_10v10_formation(host.battle_state)
		and not ally_actor.is_empty()
		and not enemy_actor.is_empty()
		and str(ally_actor.get("serverUsername", "")) == opponent_username
		and str(enemy_actor.get("serverUsername", "")) == challenger_username
		and host.battle_command_panel != null
		and host.battle_command_panel.visible
		and (host.encounter_panel == null or not host.encounter_panel.visible)
	)
	var actor_ids = host._auto_server_battle_duel_actor_ids(restored_room, challenger_username, opponent_username)
	var challenger_player_actor_id = str(actor_ids.get("challengerPlayer", ""))
	var challenger_pet_actor_id = str(actor_ids.get("challengerPet", ""))
	var opponent_player_actor_id = str(actor_ids.get("opponentPlayer", ""))
	var opponent_pet_actor_id = str(actor_ids.get("opponentPet", ""))
	...
	var opponent_pet_command_parsed = ServerAuthClientModel.parse_battle_command_response(int(opponent_pet_command_response.get("responseCode", 0)), opponent_pet_command_response.get("body", PackedByteArray()) as PackedByteArray)
	var remote_turn = opponent_pet_command_parsed.get("turn", {}) as Dictionary if opponent_pet_command_parsed.get("turn", {}) is Dictionary else {}
	if str(remote_turn.get("kind", "")) == "battle_event_list":
	...
	var updated_ally_actor = BattleModel.actor_by_id(host.battle_state, BattleModel.PLAYER_ACTOR_ID)
	var playback_ok = (
		host.battle_active
		and BattleModel.uses_10v10_formation(host.battle_state)
		and host.battle_last_round_applied_events >= 4
		and host.battle_last_round_event_types.has("attack")
		and host.battle_last_round_event_types.has("defend")
	)
	var turn_ok = (
		bool(challenger_command_parsed.get("ok", false))
		and bool(challenger_pet_command_parsed.get("ok", false))
		and bool(opponent_pet_command_parsed.get("ok", false))
		and not host.server_battle_command_request_active
		and str(last_event_list.get("kind", "")) == "battle_event_list"
	)
	var updated_ok = host.battle_active and str(host.battle_state.get("serverRoomId", "")) == room_id
	var hp_sync_ok = (
		updated_ok
		and int(host.battle_state.get("round", 0)) == 2
		and int(updated_ally_actor.get("hp", 0)) < int(updated_ally_actor.get("maxHp", 0))
		and str(last_event_list.get("kind", "")) == "battle_event_list"
		and host.battle_command_panel != null
		and host.battle_command_panel.visible
	)
	var status = "ok" if register_ok and positions_ok and room_ready_ok and state_ok and visible_ok and actor_ids_ok and turn_ok and playback_ok and hp_sync_ok else "failed"
	print("server battle reconnect live check ready: status=%s register=%s positions=%s room=%s state=%s visible=%s actors=%s turn=%s playback=%s hp_sync=%s room_id=%s challenger=%s opponent=%s events=%s" % [
		status,
		str(register_ok),
		str(positions_ok),
		str(room_ready_ok),
...
func _run_auto_server_battle_close_live_check() -> void:
	host.profile_save_enabled = false
	var suffix = str(Time.get_ticks_usec() % 10000000000)
	var challenger_username = "bca%s" % suffix
	var opponent_username = "bcb%s" % suffix
	challenger_username = challenger_username.substr(0, mini(20, challenger_username.length()))
	opponent_username = opponent_username.substr(0, mini(20, opponent_username.length()))
	var challenger_register = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
	...
	var opponent_session = opponent_parsed.get("session", {}) as Dictionary if opponent_parsed.get("session", {}) is Dictionary else {}
	var register_ok = bool(challenger_parsed.get("ok", false)) and bool(opponent_parsed.get("ok", false))
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(7, -1)
	var challenger_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(challenger_session.get("serverSessionToken", "")),
		{
...
func _run_auto_server_battle_target_mapping_check() -> void:
	...
	var closed_event_open_room = {
		...
		"battle": {
			"round": 1,
			"phase": "command",
			"requiredActorIds": ["closed_event_ally"],
			"submittedActorIds": [],
			"actors": [
				{
					"actorId": "closed_event_ally",
					"accountId": "acc_b",
					"username": "attacker_owner",
					"displayName": "普通测试号",
					"side": "ally",
	...
	host.current_account_session = {
		...
		"username": "attacker_owner",
		"authSource": ServerAuthClientModel.SOURCE_SERVER,
		"serverSessionToken": "token_closed_event",
		"serverBaseUrl": ServerAuthClientModel.DEFAULT_BASE_URL,
	}
	host.account_authenticated = true
	host.server_battle_state.clear()
	...
	closed_event_battle["lastEventList"] = {
		...
		"turnSeq": 1,
		"events": [{
			"eventId": "closed_event_pve_room:r1:e1",
			"eventType": "basic_attack",
			"round": 1,
			"sequence": 1,
			"actorAccountId": "acc_b",
			...
			"actorId": "closed_event_ally",
			"actorKind": "player",
			"targetActorId": "closed_event_enemy",
			"targetKind": "wild_pet",
			"actionId": "attack",
			"damage": 12,
			"hpBefore": 12,
			"hpAfter": 0,
			"defeated": true,
			"message": "普通测试号 攻击了 野生乌力，造成 12 点伤害。",
		}],
...
func _run_auto_server_battle_pet_snapshot_live_check() -> void:
	...
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(8, -1)
	var challenger_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(challenger_session.get("serverSessionToken", "")),
		{
...
func _run_auto_server_battle_pet_command_live_check() -> void:
	...
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(10, -1)
	var challenger_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(challenger_session.get("serverSessionToken", "")),
		{
	...
	print("server battle pet command live check ready: status=%s register=%s register_result=%d/%d profile=%s positions=%s stream=%s invite=%s room_apply=%s ready=%s visible=%s remote=%s player_to_pet=%s shared_countdown=%s countdown_after=%.2f pet_button=%s target_mode=%s click=%s event=%s hp_sync=%s local_event=%s round=%s damage=%d room_id=%s challenger=%s opponent=%s enemy_pet_hp=%d updated_enemy_pet_hp=%d" % [
		status,
		str(register_ok),
...
func _run_auto_server_battle_switch_pet_live_check() -> void:
	...
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(12, -1)
	var challenger_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(challenger_session.get("serverSessionToken", "")),
		{
...
func _run_auto_server_battle_item_live_check() -> void:
	host.profile_save_enabled = false
	var suffix = str(Time.get_ticks_usec() % 10000000000)
	var challenger_username = "bia%s" % suffix
	var opponent_username = "bib%s" % suffix
	challenger_username = challenger_username.substr(0, mini(20, challenger_username.length()))
	opponent_username = opponent_username.substr(0, mini(20, opponent_username.length()))
	var challenger_register = await host._auto_http_request_spec(ServerAuthClientModel.register_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
	...
	var battle_cell = IsoMapModel.spawn_cell(host.map_data) + Vector2i(13, -1)
	var challenger_position_response = await host._auto_http_request_spec(ServerAuthClientModel.player_position_update_request(
		ServerAuthClientModel.DEFAULT_BASE_URL,
		str(challenger_session.get("serverSessionToken", "")),
		{
