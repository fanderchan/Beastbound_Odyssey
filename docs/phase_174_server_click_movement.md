...
- 如果服务器拒绝 step，例如起点不匹配，客户端会停止当前路径并提示“位置已同步，请重新点击。”
...
1. Phase175：把服务器 `battle_event_list` 接入共享 10v10 战斗播放模板。
2. Phase176：切磋房间离开、取消、超时和战斗结果回写。
3. Phase177：逐步把地图碰撞、玩家碰撞和移动速度校验服务端化。
