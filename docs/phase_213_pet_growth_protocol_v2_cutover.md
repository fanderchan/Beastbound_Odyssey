...
> 后续更正（2026-07-11）：本阶段把“按已观察成长外推 Lv140”误当成隐藏成长泄露，因而移除了玩家需要的预测列。Phase219 已恢复只使用 Lv1、当前四项和等级的实测预测；仍禁止读取隐藏 roll 或未来逐级结果。以 `docs/phase_219_observed_lv140_forecast.md` 为当前产品边界。
...
联网成长页当时将“预测140”改为“观察趋势”，目标值显示“观察中/不预判”。该决定后来被确认错误：基于 Lv1→当前实测平均成长的预测不是隐藏 roll 透视，且是约 Lv20 判断去留的必要反馈。Phase219 已恢复实测预测，同时继续禁止客户端读取隐藏 roll 或精确未来结果。
