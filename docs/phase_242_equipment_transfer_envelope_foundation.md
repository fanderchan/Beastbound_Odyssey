...
> Phase244 后续修正：这里曾假定业务容器在单次事务中“读取→导入→删除”即可永久防重放。E1→E2 多跳后恢复旧容器的固定复现证明该假设不足；当前规则已改为根级 append-only 消费墓碑，并由独立 MySQL 增量表永久保留。墓碑不放入玩家档案，也不向客户端公开。详见 `docs/phase_244_market_mail_equipment_escrow.md`。
