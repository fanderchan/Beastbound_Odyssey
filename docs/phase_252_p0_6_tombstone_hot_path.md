...
- `server_state` 计数使用已验证 index 的常数时间 count；账本引用真正变化时仍执行 append-only 差异校验，删除和改写继续在事务前拒绝。
- 本阶段没有改变 SQL schema、档案 schema、客户端协议或玩家文案。

## 非目标与剩余风险

- 新增墓碑本身当前仍需 copy-on-write 全量对象；20,000 条 durable receipt 也仍会 normalize/prune。P0.6a-2 将改为 touched-row/append delta dirty journal，并补真实 MySQL planner 门槛。
