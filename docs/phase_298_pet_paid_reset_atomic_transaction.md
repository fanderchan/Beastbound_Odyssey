...
本页“合法一/二转宠”和“保留进化血统后回到0转”的旧合同已经撤销。现行原子事务只接受资料完整的 `authority-v1` 普通恰好一转、未终局实例；`rebirthCount>=2`、进化 lineage/目标形态或融合 lineage 均在报价、选择钱包和修改候选前失败关闭。终局拒绝时宠物、四钱包、永久次数、审计、profile revision 与 durable receipt 全部零变化，GM 覆盖不能重开资格。

普通二转和进化保留一转胚子价值；未来融合数值独立于三只材料宠，只允许白名单技能遗传。完整修正见 [Phase 354](phase_354_pet_terminal_reset_and_quality_contract.md)。
