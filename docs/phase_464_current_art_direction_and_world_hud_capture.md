...
> **Phase 465 更正（2026-08-17）**：本阶段战斗片确实来自真实 `Main.tscn` 和正式绘制器，但录像控制器
> 为做“单一最大人物／宠物动作包 + 最长名字”的极限压力测试，主动把 10 名人形全部改成焰芽斗士、把
> 10 只宠物全部改成晶甲乌力。因此下文“普通 10v10 同模复制明显”的判断不能代表正式服务端阵容，
> 只能描述该测试夹具。Phase 465 已改用 4 套当前运行人物和 3 种当前运行宠物的代表性混合阵容，并保留
> 20 席、最长名字、相邻目标与 HUD 安全区压力；新的真实 Main 证据证明克隆军团问题属于审片夹具误导，
> 不是产品数据合同缺失。完整纠错与当前真实比例问题见
> `docs/phase_465_representative_battle_art_hierarchy.md`。
