...
但第 9 回合的录像中战场背景曾变为灰底。后续逐帧更正：该回合早期 `arenaEvidence` 仍存在，但 `13153～13329` 帧的连续 12 次采样已经为空；首次丢失比服务端 `battle.room_closed` 记录晚约 83ms。最初仅查看前两次采样而写出的“当时仍返回素材”不准确。相关帧保留于上述摘要目录的 `both-downed-late.png`，继续追查结束快照对最后一轮动画的影响。
