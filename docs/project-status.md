...
2. **洞穴守护战到村口已完成自动与真实鼠标返程**：[Phase 592](phase_592_cave_return_playthrough.md) 用一个真实 Main 加四个 HTTP 测试队友完成守护战、三场普通遭遇胜利及 F4→F3→F2→F1→火芽村，55 次权威回合全部完整播放。[Phase 596](phase_596_manual_cave_return_completed.md) 又通过真实鼠标走完守护战后返村和村口移动，实际为一胜、一超时、五逃跑，15 个回合完整播放；五账号各获地之戒 +1／石币 +259，全程单连接、洞穴贴图和 1.52× 镜头保持。两轮均使用高生命 QA 队伍，不是五真人或正常难度验收；正式静止性能与美术接受未完成，发布游标保持 `R1.W024`。
...
| 四层路线与五人守护战 | Phase 581–591 已修复换层退回网格／比例、结算同步、事件流、漏播、灰底及普通宠物大小；[Phase 592](phase_592_cave_return_playthrough.md) 完成全胜自动返村，[Phase 596](phase_596_manual_cave_return_completed.md) 完成守护战胜利后鼠标返村，五账号奖励及全部回合核对通过 | 正式截图配对与静止性能仍待完成；当前鼠标路线包含一次超时及五次逃跑，不能冒充全胜或所有者美术接受，107 项本地候选文件保留 |
...
| 原生地图性能 | [Phase 598](phase_598_world_idle_rendering.md) 的普通静止同版对照 CPU `7.75% → 3.85%`，画面逐像素一致；[Phase 599](phase_599_performance_cleanup_rendering.md) 修复采样收尾提前省略绘制后，48/48 原生样本及 36,768 帧连续性通过，绝对静止／移动和移动增量达标 | 最近完整矩阵的静止增量 `0.162–0.197ms > 0.100ms` 仍 **FAIL**。[Phase 600](phase_600_retained_actor_depth_state.md) 已减少原生排序查询耗时（约 `22.4→17.5µs`），7680 帧及跨版本像素对照通过；这是局部收益，其余静止开销和完整性能复测仍待完成 |
...
**接下来的顺序**：完整自动出洞与鼠标返村已分别在 Phase 592／596 复验，Phase 598 已接入普通静止节能，Phase 599 已补完整正式性能矩阵，Phase 600 又复用人物排序状态并完成局部前后对照。继续定位并处理其余静止增量；源码稳定后补精确地图动作和截图配对，再进入 R1.W025 受委托复审。仅靠贴图范围通知的遮挡缓存方案因遗漏翻转／裁剪／原纹理尺寸变化已放弃，不重做该方案。候选显示不代表素材正式发布，原有 107 项候选文件保留。
