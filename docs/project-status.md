...
4. **依次推进成品门槛**：继续 R1.W025 受委托复审；[Phase 604](phase_604_battle_outcome_lifecycle.md) 已修复旧奖励跨战斗残留、取消竞态和长队列叠字，接着审看剩余视觉问题并重新冻结当前运行证据，再推进剩余视听验收、首发内容、玩法/经济、运营基础设施、正式构建和封测；每次交付都带具体效果或可复现结果。

下面按当前问题汇总证据。详细测试数字和当时失败记录保留在 Phase 文档；旧记录中的“下一步”不再作为当前游标。

| 当前事项 | 已有证据 | 仍需完成 |
| --- | --- | --- |
| 人物比例、遮挡与地标构图 | [Phase 555](phase_555_world_prop_player_visibility.md)／[Phase 556](phase_556_guardian_navigation_camera.md) 修复挡人与裁切；[Phase 602](phase_602_current_cave_evidence.md) 用当前 Compatibility 路径重新实测四层墙前后、石柱阻挡、上下楼和守护对话，20 组正式截图／动作已配对 | 所有者接受仍待完成；外围环境物件不能冒充全部无遮挡 |
| 四层路线与五人守护战 | Phase 581–591 修复网格／比例、结算、事件流、漏播、灰底及宠物大小；Phase602 完成当前后端四层鼠标往返，Phase603 完成当前后端守护战、两场普通遭遇全胜及逐层返村 | Phase603 完整返村为高生命 QA 自动操作；Phase604 后运行指纹已变化，当前版本精确证据待重冻；正常难度、五真人联机和所有者美术接受待完成；旧鼠标路线仍如实记录超时和逃跑 |
...
**接下来的顺序**：Phase601 完成默认后端及性能验证，Phase602 冻结当前地图动作／画面／性能，Phase603 补齐同一运行内容下的完整守护战、普通遭遇与返村。R1.W024 已完成，当前 R1.W025 的旧奖励跨战斗问题及长队列叠字已由 Phase604 修复，23 项回归与原生对照通过。继续整理四层和地标的完整视觉结论；PFC 改动使运行指纹变化，Phase601～603 的冻结原件保留为当时版本，后续需按新指纹成组重冻，不能把旧证据改标为当前。原有 107 项候选文件已完整备份后更新，旧失败回执保留；候选显示和测试通过均不代表素材正式发布或老板批准。
