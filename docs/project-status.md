...
2. **洞穴守护战到村口已完成自动与真实鼠标返程**：[Phase 592](phase_592_cave_return_playthrough.md) 用一个真实 Main 加四个 HTTP 测试队友完成守护战、三场普通遭遇胜利及 F4→F3→F2→F1→火芽村，55 次权威回合全部完整播放。[Phase 596](phase_596_manual_cave_return_completed.md) 又通过真实鼠标走完守护战后返村和村口移动，实际为一胜、一超时、五逃跑，15 个回合完整播放；五账号各获地之戒 +1／石币 +259，全程单连接、洞穴贴图和 1.52× 镜头保持。两轮均使用高生命 QA 队伍，不是五真人或正常难度验收；当前默认渲染路径的四层性能已在 Phase601 通过，精确证据配对与美术接受未完成，发布游标保持 `R1.W024`。
...
4. **依次推进成品门槛**：继续 R1.W024 的当前源码地图动作、截图及新渲染路径证据配对，再推进剩余视听验收、首发内容、玩法/经济、运营基础设施、正式构建和封测；每次交付都带具体效果或可复现结果。
...
| 四层路线与五人守护战 | Phase 581–591 已修复换层退回网格／比例、结算同步、事件流、漏播、灰底及普通宠物大小；[Phase 592](phase_592_cave_return_playthrough.md) 完成全胜自动返村，[Phase 596](phase_596_manual_cave_return_completed.md) 完成守护战胜利后鼠标返村，五账号奖励及全部回合核对通过 | 当前渲染路径的正式截图配对仍待完成；四层性能已在 Phase601 通过，当前鼠标路线包含一次超时及五次逃跑，不能冒充全胜或所有者美术接受，107 项本地候选文件保留 |
...
| 原生地图性能 | [Phase 601](phase_601_compatibility_renderer.md) 默认改用 Compatibility；最新 48/48 原生样本、36,768 帧全部连续绘制，绝对静止／移动及增量门槛全部 **PASS**，静止增量 `-0.140～-0.074ms`。普通时钟移动 CPU 同版均值 `12.596%→7.887%` | 普通静止 CPU `3.835%→4.969%` 略升；只验证本机，Windows 与多人容量未测。当前源码的地图动作、截图及性能正式配对尚未安装，老板美术接受仍待完成 |
...
**接下来的顺序**：完整自动出洞与鼠标返村已分别在 Phase 592／596 复验，Phase 598 已接入普通静止节能，Phase 600 复用人物排序状态；Phase 601 切换默认 2D 渲染后端并通过完整四层性能及 10v10 战斗性能。接下来冻结当前运行内容，补齐精确地图动作与截图，把新性能、四层片和 F4 地标片配成同一份证据，再进入 R1.W025 受委托复审。旧失败记录保留，原有 107 项候选文件保持原样；候选显示和测试通过均不代表素材正式发布或老板批准。
