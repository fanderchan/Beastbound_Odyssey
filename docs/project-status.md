...
| 原生地图性能 | Phase 572–575 已优化动画、镜头、菜单、位置和人物轮廓；[Phase 576](phase_576_cave_native_cpu_and_full_matrix.md) 修改前最近完整 48 组零失焦／零不可绘制，四层绝对静止 `0.316–0.333ms`、移动 `0.408–0.473ms` 及移动增量全部达标 | 该版静止增量 `0.150–0.169ms` 超过 `0.100ms`，总评 **FAIL**；Phase 577–578 已继续修改源码，当前完整矩阵仍待补齐，尚未安装正式性能证据 |
...
| 相机与外观的重复计算 | [Phase 577](phase_577_player_appearance_selection_cache.md) 复用未变化的外观选择；[Phase 578](phase_578_camera_policy_dispatch.md) 合并相机资格判断，384 种状态、定向 `4/4` 及跨帧移动通过。最终源码单次原生静止 600/600 帧有效，约 `7.816% CPU` | 局部及单次数据不能证明整体性能改善；完整前后对照未满足连续绘制条件，当前四层完整矩阵仍待补齐 |
...
**接下来的顺序**：局部热点已整理，先用当前源码完整原生矩阵确认剩余性能缺口，再补自动动作／真实鼠标配对及联网战斗操作。窗口可能已有焦点但停止绘制，也可能误选到另一份测试项目管理器，因此须核对目标进程和真实绘制帧。Phase 578 已回收测试遗留并请求配合稳定前台；已结束的进程与隔离目录全部清理。完整通过 R1.W024 后进入 R1.W025 受委托复审。
