...
| 原生地图性能 | [Phase 579](phase_579_current_cave_evidence.md) 四层绝对静止、移动及移动增量达标；[Phase 597](phase_597_idle_camera_render_diagnosis.md) 的正常时钟静止对照确认相机重复更新阻碍按需重绘：测试中暂停后，12 秒绘制由约 361 次降至 87 次、CPU 由 `5.335–5.838%` 降至 `2.442–2.917%`；12 项相机唤醒原型对照通过 | 优化仍为独立实验，未接入正式运行。下一步验证 Main 中的真实输入、切图与战斗唤醒；现有正式静止增量 `0.165–0.188ms > 0.100ms` 仍 **FAIL**，实验省略绘制不能替代正式矩阵 |
...
**接下来的顺序**：完整自动出洞与鼠标返村已分别在 Phase 592／596 复验，继续定位正式静止性能增量，补当前精确地图动作和截图配对，再进入 R1.W025 受委托复审。仅靠贴图范围通知的遮挡缓存方案已因遗漏翻转／裁剪／原纹理尺寸变化而放弃，不从该方案重新开始。候选显示不代表素材正式发布，原有 107 项候选文件保留。
