...
2. **洞穴守护战到村口已通过完整自动回归**：[Phase 592](phase_592_cave_return_playthrough.md) 用一个真实 Main 加四个 HTTP 测试队友，完成守护战、三场普通遭遇及 F4→F3→F2→F1→火芽村。55 次权威回合全部完整播放，五账号各获地之戒 +1，全程单连接；洞穴始终保留贴图、1.52× 镜头和 122.573–124.762px 人物轮廓。该流程使用高生命 QA 队伍和自动 viewport 输入，不是五真人、难度或 Computer Use 验收。[Phase 594](phase_594_manual_cave_return_review.md) 又用真实鼠标完成守护战、两场普通胜利及三次逃跑，59 个回合完整播放；因窗口停画停在一层，尚未返村。正式静止性能与美术接受未完成，发布游标保持 `R1.W024`。
...
| 四层路线与五人守护战 | Phase 581–591 已修复换层退回网格／比例、结算同步、事件流、漏播、灰底及普通宠物大小；[Phase 592](phase_592_cave_return_playthrough.md) 完成四场权威胜利和全部出洞路线，55 次回合无漏播／重复，五账号奖励核对通过 | Phase 594 真实鼠标已到一层，含三胜、一超时、三逃跑；仍欠返村、正式截图配对与静止性能。自动路线不替代人工接受，107 项本地候选文件保留 |
