...
| 原生地图性能 | Phase 572–575 已优化动画、镜头、菜单、位置和人物轮廓；[Phase 576](phase_576_cave_native_cpu_and_full_matrix.md) 修改前最近完整 48 组零失焦／零不可绘制，四层绝对静止 `0.316–0.333ms`、移动 `0.408–0.473ms` 及移动增量全部达标 | 该版静止增量 `0.150–0.169ms` 超过 `0.100ms`，总评 **FAIL**；Phase 577 已继续修改源码，当前完整矩阵仍待补齐，尚未安装正式性能证据 |
...
| 人物外观同步 | [Phase 577](phase_577_player_appearance_selection_cache.md) 复用未变化的外观选择，18 种换装／回退状态、定向 `4/4`、12 组 headless 及跨帧移动通过；完整脚本静止／移动中位数约下降 `3.7%／2.0%` | 原生对照第三段失焦并停止绘制，整批拒绝；不宣称 CPU 或帧率收益，当前完整前台证据仍待补齐 |
...
**接下来的顺序**：Phase 576 最近完整矩阵的未通过项收敛到静止增量；Phase 577 又减少了重复外观解析，但原生重复对照未完成。继续按脚本分析处理剩余静止开销，在稳定前台环境验证当前源码；随后补自动动作／真实鼠标配对及联网战斗操作，完成 R1.W024 后进入 R1.W025 受委托复审。窗口状态变化时先读日志再决定是否重试，不能拼接不同批次或放宽门槛；已结束的测试进程与隔离环境均已清理。
