...
**接下来的顺序**：Phase 582 修复战斗结算等待，Phase 583 修复重连计时，Phase 584 补齐后台录制；[Phase 585](phase_585_battle_event_receive_capacity.md) 又修复战斗消息超过默认接收容量导致的静默断连，当前完整自动战斗已保持单连接／零拒绝并完成五账号结算。继续真实鼠标双倒地和连续返回／出洞，再处理正式静止性能增量与当前精确证据配对，完整通过 R1.W024 后进入 R1.W025 受委托复审。候选显示不代表素材正式发布，原有 107 项候选文件保留。
