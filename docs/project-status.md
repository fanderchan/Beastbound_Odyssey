...
4. **依次推进成品门槛**：进入 R1.W025 受委托复审，先处理连续遇敌时旧奖励浮字残留，再推进剩余视听验收、首发内容、玩法/经济、运营基础设施、正式构建和封测；每次交付都带具体效果或可复现结果。
...
| 四层路线与五人守护战 | Phase 581–591 修复网格／比例、结算、事件流、漏播、灰底及宠物大小；Phase602 完成当前后端四层鼠标往返，Phase603 完成当前后端守护战、两场普通遭遇全胜及逐层返村 | 当前完整返村为高生命 QA 自动操作；正常难度、五真人联机和所有者美术接受待完成；旧鼠标路线仍如实记录超时和逃跑 |
...
**接下来的顺序**：Phase601 完成默认后端及性能验证，Phase602 冻结当前地图动作／画面／性能，Phase603 补齐同一运行内容下的完整守护战、普通遭遇与返村。R1.W024 已完成，当前 R1.W025 复审先处理实机发现的旧奖励浮字跨入下一场战斗，再整理明确的视觉结论。原有 107 项候选文件已完整备份后更新，旧失败回执保留；候选显示和测试通过均不代表素材正式发布或老板批准。
