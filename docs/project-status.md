...
| 四层路线与五人守护战 | [Phase 581](phase_581_cave_journey_visuals_and_result_input.md) 修复换层美术／比例、结算点击和队友续战。[Phase 582](phase_582_battle_response_ownership.md) 修复结算等待；[Phase 587](phase_587_downed_owner_mouse_regression.md) 已真实鼠标复验双倒地后五账号结算，单连接零拒绝，另走到二层并逃离普通遭遇 | 灰底已在 [Phase 588](phase_588_closed_battle_arena_context.md) 修复；[Phase 589](phase_589_ordered_server_battle_playback.md) 已逐项复验双倒地后全部 11 回合播放；完整一层／出洞、正式截图配对和性能仍待完成，107 项本地候选文件保留 |
...
**接下来的顺序**：战斗结算、重连计时、大事件断连、后台绘制和录像完整性已修复。[Phase 588](phase_588_closed_battle_arena_context.md) 又修复关闭房间丢失地图 ID 导致的灰底，真实鼠标双倒地复验覆盖压缩快照，五账号奖励到账。[Phase 589](phase_589_ordered_server_battle_playback.md) 已修复动画忙碌时丢弃新回合及未来快照覆盖，真实鼠标复验完整播放 11 回合后结算，五账号奖励核对通过。下一步走通一层／洞口。随后处理正式静止性能增量与当前精确证据配对，完整通过 R1.W024 后进入 R1.W025 受委托复审。候选显示不代表素材正式发布，原有 107 项候选文件保留。
