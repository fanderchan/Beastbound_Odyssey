...
4. **依次推进成品门槛**：继续 R1.W024 当前渲染路径的完整守护战／遭遇／返村流程，随后受委托复审，再推进剩余视听验收、首发内容、玩法/经济、运营基础设施、正式构建和封测；每次交付都带具体效果或可复现结果。
...
| 四层路线与五人守护战 | Phase 581–591 已修复换层网格／比例、结算、事件流、漏播、灰底及宠物大小；Phase592／596 分别完成自动全胜返村与真实鼠标返村；Phase602 完成当前后端的无遭遇四层鼠标往返 | 当前后端还需完整守护战、普通遭遇和返村配对；旧鼠标路线包含一次超时及五次逃跑，不能冒充全胜或所有者美术接受 |
...
**接下来的顺序**：Phase601 切换默认后端并通过四层及 10v10 性能；Phase602 已冻结当前地图动作、截图、性能与四层／F4 地标片，修复独立审计器的旧后端判断。接着在同一运行内容下完成守护战、普通遭遇及出洞返村，再进入 R1.W025 受委托复审。原有 107 项候选文件已完整备份后更新，旧失败回执保留；候选显示和测试通过均不代表素材正式发布或老板批准。
