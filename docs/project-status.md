...
| 原生地图性能 | [Phase 579](phase_579_current_cave_evidence.md) 完整 48 组中四层绝对静止 `0.313–0.339ms`、移动 `0.368–0.461ms` 及移动增量达标；[Phase 593](phase_593_native_performance_diagnostics.md) 补得正常设置下两段美术 `8.832% / 8.462% CPU`、约 30 FPS 的有效局部证据，并完善准备页失败诊断 | 静止增量 `0.165–0.188ms` 超过 `0.100ms`，正式总评仍 **FAIL**；新 CPU 对照第四段失焦，整批未通过。部分进程数据不能替代正式矩阵或证明已达低个位数目标 |
