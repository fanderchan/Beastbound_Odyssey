...
| 原生地图性能 | [Phase 598](phase_598_world_idle_rendering.md) 的普通静止同版对照 CPU `7.75% → 3.85%`，画面逐像素一致；[Phase 599](phase_599_performance_cleanup_rendering.md) 修复采样收尾提前省略绘制后，48/48 原生样本及 36,768 帧连续性通过，绝对静止／移动和移动增量达标 | 当前正式静止增量 `0.162–0.197ms > 0.100ms` 仍 **FAIL**。普通按需绘制收益限本机静止场景，不能替代正式矩阵；接下来定位深度层等静止回调的实际开销 |
...
**接下来的顺序**：完整自动出洞与鼠标返村已分别在 Phase 592／596 复验，Phase 598 已接入普通静止节能，Phase 599 已补当前源码的完整正式性能矩阵。继续定位并处理静止增量超标；源码稳定后补精确地图动作和截图配对，再进入 R1.W025 受委托复审。仅靠贴图范围通知的遮挡缓存方案因遗漏翻转／裁剪／原纹理尺寸变化已放弃，不重做该方案。候选显示不代表素材正式发布，原有 107 项候选文件保留。
