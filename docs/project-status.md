...
| 原生地图性能 | [Phase 571](phase_571_current_cave_input_and_performance.md) 最近可作正式判断的完整矩阵为 **FAIL**；[Phase 572](phase_572_world_animation_hotpath.md) 减少动画／镜头重复计算，[Phase 573](phase_573_panel_visibility_hotpath.md) 缓存菜单显隐；[Phase 574](phase_574_player_position_updates.md) 消除人物未越界时的重复位置更新，720 步状态一致、定向 `4/4`。当前另有 48 组完整绘制诊断，提示继续处理候选地图静止增量 | 这批诊断含测量期间的额外窗口操作，不能晋升正式证据；新批次又因锁屏未启动 Main。尚无整游戏 CPU 改善或完整达标结论，待解锁后正式复证 |
...
**接下来的顺序**：Phase 572–574 已完成动画、镜头、菜单及位置更新的重复计算优化；先比较候选地图与网格静止时的调用差异，再做有依据的修复。待用户解锁、窗口可持续留在前台后补正式矩阵及正常时钟 CPU；随后完成自动动作／真实鼠标配对与联网战斗操作，完成 R1.W024 后进入 R1.W025 受委托复审。20 项当前地图鼠标观察已补齐，窗口中断及受额外操作影响的数据均保留，正式候选尚未更新；可独立验证的工作继续按授权推进。
