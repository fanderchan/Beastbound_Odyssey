...
| 原生地图性能 | [Phase 598](phase_598_world_idle_rendering.md) 已接入静止相机休眠与按需绘制：同版原生正常时钟对照 CPU `7.75% → 3.85%`，约减少 50%，四张画面逐像素一致；120 次跨帧点击、四层切换和战斗往返通过，联机面板复验 4066/4066 帧连续绘制 | 收益限于本机该静止场景；新源码完整 48 组正式矩阵尚未重跑，Phase 579 正式静止增量 `0.165–0.188ms > 0.100ms` 仍 **FAIL**。普通按需绘制样本不能替代正式矩阵 |
...
**接下来的顺序**：完整自动出洞与鼠标返村已分别在 Phase 592／596 复验，Phase 598 已接入并验证普通静止节能。接下来重测新源码正式地图性能，继续处理不达标的静止增量，补当前精确地图动作和截图配对，再进入 R1.W025 受委托复审。仅靠贴图范围通知的遮挡缓存方案已因遗漏翻转／裁剪／原纹理尺寸变化而放弃，不从该方案重新开始。候选显示不代表素材正式发布，原有 107 项候选文件保留。
