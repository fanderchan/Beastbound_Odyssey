...
**接下来的顺序**：战斗结算、重连计时、大事件断连、后台绘制和录像完整性已修复。[Phase 588](phase_588_closed_battle_arena_context.md) 又修复关闭房间丢失地图 ID 导致的灰底，真实鼠标双倒地复验覆盖压缩快照，五账号奖励到账。[Phase 589](phase_589_ordered_server_battle_playback.md) 已修复动画忙碌时丢弃新回合及未来快照覆盖，真实鼠标复验完整播放 11 回合后结算，五账号奖励核对通过。[Phase 590](phase_590_authoritative_battle_timeout.md) 已修复联网超时产生本地虚假回合，Godot 6/6、Node 4/4、Python 13/13；新实机鼠标因 `noWindowsAvailable` 未能继续。下一步核查普通遭遇宠物比例，并在窗口可操作时走通一层／洞口。随后处理正式静止性能增量与当前精确证据配对，完整通过 R1.W024 后进入 R1.W025 受委托复审。候选显示不代表素材正式发布，原有 107 项候选文件保留。
