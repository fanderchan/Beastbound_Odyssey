...
**接下来的顺序**：Phase 572–575 已完成有局部对照支持的运行优化；四层 headless 调用差异也已确认。下一步在可操作窗口中补原生网格／美术脚本归因，定位剩余开销，再完成前台矩阵与正常时钟 CPU 对照；随后补自动动作／真实鼠标配对及联网战斗操作，完成 R1.W024 后进入 R1.W025 受委托复审。当前原生批次仅前 32 组有效，第 33 组失焦使整批被拒绝；没有拼接失败批次或更新正式候选，独立可验证的工作继续按授权推进。
