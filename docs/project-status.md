...
2. **洞穴守护战到村口已通过完整自动回归**：[Phase 592](phase_592_cave_return_playthrough.md) 用一个真实 Main 加四个 HTTP 测试队友，完成守护战、三场普通遭遇及 F4→F3→F2→F1→火芽村。55 次权威回合全部完整播放，五账号各获地之戒 +1，全程单连接；洞穴始终保留贴图、1.52× 镜头和 122.573–124.762px 人物轮廓。该流程使用高生命 QA 队伍和自动 viewport 输入，不是五真人、难度或 Computer Use 验收。当前真实鼠标仍受窗口工具不可用影响，正式静止性能与美术接受尚未完成，发布游标保持 `R1.W024`。
...
| 四层路线与五人守护战 | Phase 581–591 已修复换层退回网格／比例、结算同步、事件流、漏播、灰底及普通宠物大小；[Phase 592](phase_592_cave_return_playthrough.md) 完成四场权威胜利和全部出洞路线，55 次回合无漏播／重复，五账号奖励核对通过 | 当前 Computer Use 完整返程、正式截图配对和静止性能仍待完成；自动路线不替代人工接受，107 项本地候选文件保留 |
...
**接下来的顺序**：连续战斗、换层比例和完整自动出洞已在 [Phase 592](phase_592_cave_return_playthrough.md) 复验；本轮没有修改游戏运行逻辑。继续定位正式静止性能增量，在窗口可操作时补 Computer Use 完整返程和当前精确证据配对，再进入 R1.W025 受委托复审。仅靠贴图范围通知的遮挡缓存方案已因遗漏翻转／裁剪／原纹理尺寸变化而放弃，不从该方案重新开始。候选显示不代表素材正式发布，原有 107 项候选文件保留。
