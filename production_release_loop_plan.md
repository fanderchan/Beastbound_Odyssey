...
- [ ] **R1.W024 AUTO｜Earth Vein 录片资源收口与精确证据重冻**
  ...
  战斗优化与结算修复（2026-09-18）：排序由每次 380 次站位解析降为 20 次，动画元数据/目录缓存及 exact-form 权限失效检查通过；修复人物击飞导致联网战斗提前胜利，真实五人队伍继续到第九回合权威结算，五账号 revision `102→103` 且各获地之戒。定向两组 `4/4 + 6/6`、性能探针 `5/5`，首次准备单次 `1169→1002ms`；严格原生性能因结束时失焦未通过，保留失败证据，仍不勾选。后续优先首次加载和同条件性能复测，见 Phase 548。
