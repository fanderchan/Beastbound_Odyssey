...
- [ ] **R1.10 AUTO｜执行 Earth Vein Cave v1 验收结论**
  依赖：R1.W025。批准则通过地图提升工具发布并验证 hash/provenance，以及普通启动下的实际视觉与比例；若决定洞穴内容首发延期，先明确任务、练级、地之戒／MM2 依赖和已有角色位置安排，再协调客户端入口／路线与服务端访问，证明区域不可达。Phase610 已确认当前美术关闭仍允许 207 步、8 次合法传送完成四层往返，不能把 `runtimeEnabled=false` 或 preview PASS 当作已执行延期；首发可达内容不能继续混用占位。
...
| R1 历史候选验收 | 进行中（R1.W025 当前证据与受委托审看已齐，建议内部试玩冻结，待所有者首发采用／返工结论） | R1.19 完成 |
