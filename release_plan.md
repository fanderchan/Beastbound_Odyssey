...
- [ ] 阶段 A 验收：全量自动检查 + 性能双基线 + 用户确认
  - 证据（2026-07-03）：验证已完成，待用户确认后勾选；`npm test --prefix server/node` 通过（76/76），`godot --headless --path client/godot --quit` 与 `git diff --check` 通过；最终全量 Godot 自动检查 `.run/stage_a_godot_auto_checks_full2.log` 覆盖 183/183，汇总 `passed_count=183 failed=null elapsed=1135.03s`；idle `--perf-probe` headless 对照 `ps` 为 1.4/1.5/1.8% 且 `process_total≈0.30-0.38ms`，GUI 窗口对照 `ps≈35.7-36.8%` 但 `process_total≈0.23-0.40ms`（定位为窗口渲染层成本而非脚本热路径）；moving `--movement-perf-check --perf-probe` 通过，`ps` 为 4.3/3.4%，`process_total≈0.26-0.33ms`；真实输入 `--movement-spam-click-check --perf-probe` 通过，360 次点击合并为 19 次路径应用，`avg_input_us=35 max_input_us=204`。

### 阶段 B
- [ ] B1 认证速率限制/失败退避/密码策略/会话刷新
- [ ] B2 默认存储切 MySQL，写路径异步化
- [ ] B3 崩溃恢复与 session_expired 引导
- [ ] B4 协议版本协商
- [ ] B5 结构化日志 + /health 扩展
- [ ] 阶段 B 验收：测试全绿 + 用户确认
