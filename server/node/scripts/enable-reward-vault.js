...
function parseArgs(argvValue = []) {
  ...
  if (argv.some(isCredentialArgument)) {
    throw commandError("reward_vault_feature_credential_argument_denied");
  }
  if (argv.length === 1 && argv[0] === "--enable") {
    throw commandError("reward_vault_feature_maintenance_confirmation_required");
  }
  if (
    argv.length === 2
    && new Set(argv).size === 2
  ...
  throw commandError("reward_vault_feature_argument_invalid");
}
...
async function runMain(argv = process.argv.slice(2), dependencies = {}) {
  ...
  try {
    ...
    return failureReport(String(error && error.code || "reward_vault_feature_argument_invalid"));
  }
  let store = null;
  try {
    ...
    if (!store || typeof store.enableRewardVaultFeature !== "function") {
      throw commandError("reward_vault_feature_executor_invalid");
    }
    const report = await store.enableRewardVaultFeature({
...
function safeFailureCode(error) {
  ...
  return /^(?:reward_vault_feature_|mysql_(?:mail_storage|pool|session|transaction|commit)_)/.test(code)
    ? code
    : "reward_vault_feature_enable_failed";
}
...
if (require.main === module) {
  runMain().then((report) => {
    ...
    printJson(failureReport("reward_vault_feature_enable_failed"));
    process.exitCode = 1;
  });
}
