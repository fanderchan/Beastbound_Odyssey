...
test("four character slots isolate profiles and require selection once the account has multiple characters", () => {
  ...
  const second = service.createCharacter(registered.session.token, {
    slotIndex: 1,
    displayName: "角色乙",
  });
  ...
  assert.equal(service.createCharacter(registered.session.token, {
    slotIndex: 2,
    displayName: "角色丙",
  }).ok, true);
  assert.equal(service.createCharacter(registered.session.token, {
  ...
  const fifth = service.createCharacter(registered.session.token, {displayName: "角色戊"});
...
test("character selection fails closed while account-scoped asset escrow is active", () => {
  ...
  const created = seedService.createCharacter(registered.session.token, {
    slotIndex: 1,
    displayName: "托管乙",
  });
  assert.equal(created.ok, true);
...
test("an unselected multi-character session cannot replay a legacy account-only asset receipt", async () => {
  ...
  assert.equal(seedService.createCharacter(registered.session.token, {
    slotIndex: 1,
    displayName: "回执乙",
  }).ok, true);
...
test("HTTP character routes enforce durable create and rotate into the selected profile", async (t) => {
  ...
  const missingKey = await fetchJson(`${baseUrl}/characters`, {
    ...
    body: JSON.stringify({slotIndex: 1, displayName: "网络乙"}),
  });
