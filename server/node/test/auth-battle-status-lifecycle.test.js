...
function statusFriendlyAuthority() {
  const rooms = new Set();
  const contexts = [];
  return {
    contexts,
...
test("sleep wakes on direct positive damage while stone keeps its exact action-opportunity decrement", () => {
  ...
  challengerProfile.petInstances[0].activeSkillIds = ["pet_attack", "pet_defend", "pet_sleep_powder", "pet_stone_gaze"];
  challengerProfile.petInstances[0].petSkillSlots = ["pet_attack", "pet_defend", "pet_sleep_powder", "pet_stone_gaze", "", "", ""];
  challengerProfile.backpackSlots = [{itemId: "item_poison_single_5", count: 1}];
  const opponentProfile = battleProfile("状态生命乙", {
    level: 20, hp: 300, maxHp: 300, attack: 20, defense: 8, quick: 100, comboRateOverride: 0,
  }, {
