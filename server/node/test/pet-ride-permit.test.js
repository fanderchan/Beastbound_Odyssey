...
test("new profiles require the configured Bui permit and unlock it once", () => {
  ...
  const planned = planUnlock(profile, RIDING, "bui_novice_sprout_taming_certificate");
  ...
  const duplicate = planUnlock(profile, RIDING, "bui_novice_sprout_taming_certificate");
...
test("malformed or mismatched permit state fails closed", () => {
  ...
  assert.equal(planUnlock(malformed, RIDING, "bui_novice_sprout_taming_certificate").code, "ride_permit_state_invalid");
