    ...
    - [ ] **P0.6d-2 profile/listing/mail/envelope 细粒度锁**：共享事务 harness、资源条件更新、affectedRows、canonical lock order、竞争/死锁/模糊 COMMIT 与并行门槛。
      ...
      - [ ] **P0.6d-2c listing/mail/envelope 与跨账号锁序**：拆资源条件更新、canonical 多行锁序、deadlock/timeout/模糊 COMMIT 与跨账号资产对账。
        ...
        - [ ] **P0.6d-2c-4 跨账号统一门槛**：canonical 多账号/多资源锁序、deadlock/timeout、隔离真实 MySQL 竞争、市场/邮件跨 Node 可见性与最终资产对账。
          ...
          - [ ] **P0.6d-2c-4c 数据库锁等待与事务期限**：先把 session 级 row/metadata lock wait 限制在 durable 响应期限内，再补 pool acquire、query/transaction hard deadline、确定回滚与模糊 COMMIT 分类；HTTP 断线只取消未开始的安全工作。
