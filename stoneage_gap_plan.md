...
- [ ] **P0.6 200 人同地图容量与安全基线**：除地图/事件/战斗压力外，加入 200 档案与 5万/10万永久装备墓碑门槛；把全根扫描改为增量索引/dirty cache，战斗和普通资产写入不得随历史墓碑线性退化；横向多 Node 前补数据库 CAS/行锁。
  ...
  - [ ] **P0.6b 200 人同 AOI 增量 presence 与 WS 背压**：把完整 roster fanout 改成地图/AOI 桶和单角色位置增量；只读 presence 退出资产 candidate；出站预算、合并和慢消费者断开有界，并用 200 个真实 loopback HTTP/WS 连接验证。
