...
`--autoplay` 还要求事件流在首次 ready 后保持健康、没有重试，且服务端仅接受一个连接；单靠 HTTP 战斗结算或无 ERROR 日志不足以通过。该条件只用于没有故意断网的本机夹具。大事件容量修复与真实回环边界见 [Phase 585](../docs/phase_585_battle_event_receive_capacity.md)。
