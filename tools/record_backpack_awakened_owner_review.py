#!/usr/bin/env python3
"""Record and validate the real awakened backpack owner-review flow.

The recorder launches the production ``Main.tscn`` path with a fresh Godot
user-data directory.  It never starts a backend and never reads or writes the
normal player save.  The Godot capture controller performs the player-facing
left-click sequence and exits non-zero when an expected interaction fails.
"""
...
CORE_SPEC = importlib.util.spec_from_file_location(
    "_beastbound_owner_review_media_core",
    CORE_PATH,
)
...
DEFAULT_CAPTURE_FLAG = "--backpack-awakened-owner-review-capture"
DEFAULT_OUTPUT_ROOT = Path(
    ".run/evidence/phase377_backpack_awakened_owner_review"
)
REPORT_SCHEMA_VERSION = 1
REPORT_TYPE = "beastbound_backpack_awakened_owner_review_video"
EXPECTED_WIDTH = 1280
EXPECTED_HEIGHT = 720
EXPECTED_FPS = 30
...
END_MARKER = "BACKPACK_AWAKENED_OWNER_REVIEW_END"
CHAPTER_MARKER = "BACKPACK_AWAKENED_OWNER_REVIEW_CHAPTER"
EXPECTED_CHAPTERS = (
    "world",
    "backpack_overview",
    "slot_capacity",
    ...
    "filter_capture",
    "filter_equipment",
    "exact_instance_comparison",
    "exact_instance_equipped",
    "gain_loss_comparison",
    "equipped_detail",
    "unequip_result",
    "pet_egg_headshot",
    "ride_permit_headshot",
    "return_world",
)


class BackpackAwakenedRecordingError(RuntimeError):
    """The backpack owner-review recording contract was not satisfied."""
...
def _new_run_id() -> str:
    ...
    return f"phase377-{timestamp}-{uuid.uuid4().hex[:8]}"
...
def _validate_probe(probe: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise BackpackAwakenedRecordingError(str(error)) from error
    ...
    if (
        ...
        raise BackpackAwakenedRecordingError(
            "背包验收视频时长必须在 "
...
def _validate_godot_log(path: Path) -> dict[str, Any]:
    ...
    if "BACKPACK_AWAKENED_OWNER_REVIEW_FAILED" in text:
        raise BackpackAwakenedRecordingError(
            "Godot 背包验收序列报告失败，详见录制日志"
        )
    ...
    if "speed=1.00x" not in text:
        raise BackpackAwakenedRecordingError(
            "Godot 背包验收序列没有声明 1× 时间"
        )
    if "profile=isolated backend=false" not in text:
        raise BackpackAwakenedRecordingError(
            "Godot 背包验收序列没有确认隔离档案和离线后端"
        )
...
def _record_into(
    *,
    ...
    avi_path = run_dir / "backpack-awakened-owner-review-1x.avi"
    video_path = run_dir / "backpack-awakened-owner-review-1x.mp4"
    ...
    requested_times = tuple(args.sample_times or ())
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "用真实 Main.tscn 录制 1280x720、30fps、1×、有声的觉醒风"
            "背包验收视频，并生成 MP4、联系表、元数据与完整解码证据。"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "backpack awakened owner review recording interrupted",
            file=sys.stderr,
        )
        return 130
    except (
        BackpackAwakenedRecordingError,
        ...
        print(
            f"backpack awakened owner review recording failed: {error}",
            file=sys.stderr,
        )
        return 1
    return 0
