#!/usr/bin/env python3
"""Record the real ``Main.tscn`` hang-matchmaking flow at deterministic 1x.

The isolated SceneTree runner uses real left-click events and feeds only valid
authoritative-state fixtures through the client's deterministic controller
hook.  It never starts a backend and never claims that fixture counts are live
...
CORE_SPEC = importlib.util.spec_from_file_location(
    "_beastbound_hang_matchmaking_media_core",
    CORE_PATH,
)
...
REPORT_TYPE = "beastbound_hang_matchmaking_owner_review_video"
EXPECTED_WIDTH = 1280
EXPECTED_HEIGHT = 720
EXPECTED_FPS = 30
...
MIN_DURATION_SECONDS = 22.0
MAX_DURATION_SECONDS = 36.0
DEFAULT_SAMPLE_COUNT = 15
MAX_SAMPLE_COUNT = 16
SAFE_RUN_ID = re.compile(r"^[A-Za-z0-9][A-Za-z0-9._-]{0,127}$")

START_MARKER = "HANG_MATCHMAKING_OWNER_REVIEW_START"
CHAPTER_MARKER = "HANG_MATCHMAKING_OWNER_REVIEW_CHAPTER"
STATE_MARKER = "HANG_MATCHMAKING_OWNER_REVIEW_STATE"
END_MARKER = "HANG_MATCHMAKING_OWNER_REVIEW_END"
FAILURE_MARKER = "HANG_MATCHMAKING_OWNER_REVIEW_FAILED"
EXPECTED_CHAPTERS = (
    ("world_context", 1.5),
    ("route_cards", 2.3),
    ("party_overview", 2.0),
    ("start_choice", 2.2),
    ("human_priority_queue", 2.6),
    ("npc_fill", 2.6),
    ("human_replacement", 2.8),
    ("world_matching_status", 2.3),
    ("cancelled_match_hang_continues", 2.4),
    ("stopped_hang", 1.8),
)
...
class HangMatchmakingRecordingError(RuntimeError):
    """The hang-matchmaking owner-review contract was not met."""
...
def _new_run_id() -> str:
    ...
    return f"phase394-{timestamp}-{uuid.uuid4().hex[:8]}"
...
def _validate_probe(probe: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise HangMatchmakingRecordingError(str(error)) from error
...
def _validate_godot_log(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise HangMatchmakingRecordingError(
            "Godot 挂机匹配验收没有确认 1280×720 30fps Movie Maker"
        )
    if FAILURE_MARKER in text:
        raise HangMatchmakingRecordingError("Godot 挂机匹配验收序列报告失败")
    for marker in (START_MARKER, STATE_MARKER, END_MARKER):
        if marker not in text:
            raise HangMatchmakingRecordingError(
                f"Godot 挂机匹配验收缺少标记：{marker}"
            )
    start_line = next(
    ...
    state_pattern = re.compile(
        ...
        + r"\s+server_writes=(\d+)\s+online_claims=(true|false)"
        + r"\s+max_process_ms=([0-9.]+)"
    )
    state_match = state_pattern.search(text)
    if state_match is None:
        raise HangMatchmakingRecordingError(
            "Godot 挂机匹配验收缺少完整状态摘要"
        )
    ...
    online_claims = groups[len(EXPECTED_STATE_FLAGS) + 3]
    max_process_ms = float(groups[len(EXPECTED_STATE_FLAGS) + 4])
    if any(value != "true" for value in flag_values):
        raise HangMatchmakingRecordingError(
            "挂机匹配验收缺少路线、匹配、NPC、取消或停止状态"
        )
    if actual_clicks < 9 or press_frames != actual_clicks:
        raise HangMatchmakingRecordingError(
            "挂机匹配验收没有完整使用跨帧真实左键"
        )
    if server_writes != 0 or online_claims != "false":
        raise HangMatchmakingRecordingError(
            "挂机匹配验收报告了服务端写入或伪在线人数声明"
        )
    if not math.isfinite(max_process_ms) or max_process_ms > 80.0:
        raise HangMatchmakingRecordingError(
            "挂机匹配验收进程帧时间超过 80ms"
        )
    end_match = re.search(
    ...
    if end_match is None or end_match.group(1) != "true":
        raise HangMatchmakingRecordingError(
            "挂机匹配验收没有完整收敛到停止挂机"
        )
    return {
        ...
        "serverWriteCount": 0,
        "onlinePopulationClaims": False,
        "stateSource": "deterministic_injected_controller",
        "maxProcessMilliseconds": max_process_ms,
        "playbackSpeed": 1.0,
...
def _record_into(
    *,
    ...
    avi_path = run_dir / "hang-matchmaking-owner-review-1x.avi"
    video_path = run_dir / "hang-matchmaking-owner-review-1x.mp4"
    ...
    summary = {
        ...
        "captureContract": {
            ...
            "serverWritesAllowed": False,
            "onlinePopulationClaimsAllowed": False,
        },
        "isolation": {
...
def _record(args: argparse.Namespace) -> Path:
    ...
    if SAFE_RUN_ID.fullmatch(run_id) is None:
        raise HangMatchmakingRecordingError(f"不安全的 runId：{run_id!r}")
    try:
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "1280x720、30fps、1×、有声的挂机匹配完整验收视频。"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print("hang matchmaking owner review recording interrupted", file=sys.stderr)
        return 130
    except (
        HangMatchmakingRecordingError,
        ...
        print(
            f"hang matchmaking owner review recording failed: {error}",
            file=sys.stderr,
        )
        return 1
    return 0
