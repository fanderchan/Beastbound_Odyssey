#!/usr/bin/env python3
"""Record the real awakened market flow from production ``Main.tscn``.

Every run uses a fresh Godot ``--user-data-dir`` below the evidence directory.
The recorder never starts a backend and delegates the player-facing sequence to
the fixed ``--market-awakened-owner-review-capture`` Godot entrypoint.
"""
...
CORE_SPEC = importlib.util.spec_from_file_location(
    "_beastbound_market_awakened_media_core",
    CORE_PATH,
)
...
DEFAULT_CAPTURE_FLAG = "--market-awakened-owner-review-capture"
DEFAULT_OUTPUT_ROOT = Path(
    ".run/evidence/phase392_market_awakened_owner_review"
)
REPORT_SCHEMA_VERSION = 1
REPORT_TYPE = "beastbound_market_awakened_owner_review_video"
EXPECTED_WIDTH = 1280
EXPECTED_HEIGHT = 720
EXPECTED_FPS = 30
...
CHAPTER_MARKER = "MARKET_AWAKENED_OWNER_REVIEW_CHAPTER"
END_MARKER = "MARKET_AWAKENED_OWNER_REVIEW_END"
FAILURE_MARKER = "MARKET_AWAKENED_OWNER_REVIEW_FAILED"
EXPECTED_CHAPTERS = (
    "world_context",
    "market_buy_overview",
...
class MarketAwakenedRecordingError(RuntimeError):
    """The market owner-review recording contract was not satisfied."""
...
def _new_run_id() -> str:
    ...
    return f"phase392-{timestamp}-{uuid.uuid4().hex[:8]}"
...
def _validate_probe(probe: dict[str, Any]) -> dict[str, Any]:
    try:
        ...
        raise MarketAwakenedRecordingError(str(error)) from error
...
def _validate_godot_log(path: Path) -> dict[str, Any]:
    ...
    if "speed=1.00x" not in text:
        raise MarketAwakenedRecordingError(
            "Godot 交易所验收序列没有声明 1× 时间"
        )
    if "profile=isolated backend=false" not in text:
        raise MarketAwakenedRecordingError(
            "Godot 交易所验收序列没有确认隔离档案和离线后端"
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "交易所验收视频，并生成 MP4、联系表、元数据与完整解码证据。"
        )
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(
            "market awakened owner review recording interrupted",
            file=sys.stderr,
        )
        return 130
    except (
        MarketAwakenedRecordingError,
        ...
        print(
            f"market awakened owner review recording failed: {error}",
            file=sys.stderr,
        )
        return 1
    return 0
